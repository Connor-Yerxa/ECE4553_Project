...
def load_audio(filename, start=None, end=None, max_frames=None):
    ...
    if FLAGS.mel_spectrogram:
        mfccs = librosa.feature.melspectrogram(y=audio, sr=16000, n_mels=128, center=False, n_fft=512, win_length=432, hop_length=160).T
        mfccs = np.log(mfccs+1e-5)
    else:
        mfccs = librosa.feature.mfcc(y=audio, sr=16000, n_mfcc=26, n_fft=512, win_length=432, hop_length=160, center=False).T
...
def extract_features(x, sr=16000):
    import pyworld
    # Normalize waveform
    x = x.astype(np.float64)

    # Extract F0 using pyworld's harvest
    f0, t = pyworld.harvest(x, sr, f0_floor=60.0, f0_ceil=240.0)
    f0 = pyworld.stonemask(x, f0, t, sr)

    # Extract spectral envelope (SP) and convert to MCEP
    sp = pyworld.cheaptrick(x, f0, t, sr)
    mcep = pyworld.code_spectral_envelope(sp, sr, dim=26)  # 25-order MCEP + energy

    # Trim to match lengths
    min_len = min(len(f0), mcep.shape[0])
    f0 = f0[:min_len]
    mcep = mcep[:min_len]

    return np.concatenate([f0[:, np.newaxis], mcep], axis=1).astype(np.float32)
